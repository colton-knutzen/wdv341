...
try { 
    ...
    /* Pulling data from server test
    while($row = $stmt->fetch() ){
        echo "<p>" . $row["product_name"] . " " . $row["product_description"] . " " . $row["product_price"] . " " . $row["product_image"] . " " . $row["product_inStock"] . " " . $row["product_status"];
    }
    */

    /*Storing in variable to be called later in HTML test
    $testTable = "";
    while($row = $stmt->fetch() ){
        $testTable .= "<p>" . $row["product_name"] . " " . $row["product_description"] . " " . $row["product_price"] . " " . $row["product_image"] . " " . $row["product_inStock"] . " " . $row["product_status"];
    }*/

    $productsTable = "";
    while($row = $stmt->fetch() ){
        $productsTable .= '<div class="productBlock"> 
                            <div class="productImage"> 
                                <image src="productImages/' . $row["product_image"] . '"> 
                            </div> 
                            ...
                            if ($row["product_status"] !== ""){ //if the column in product_status is blank, it will skip this element and move on. But why does it return blank and not null?
                            $productsTable .= '<p class="productStatus">' . $row["product_status"] . '</p>';
                            }
                            
                            if ($row["product_inStock"] < 10){ //if less than 10, extra class is added
                                $productsTable .= '<p class="productInventory productLowInventory ">' . $row["product_inStock"] . ' In Stock!</p>
                                </div>';
                            }else {
                                $productsTable .= '<p class="productInventory">' . $row["product_inStock"] . ' In Stock!</p>
                                </div>'; 
                            }                        
    }    
?>
